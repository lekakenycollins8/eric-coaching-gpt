  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 403 && errorData && errorData.error === 'Subscription required') {
        console.log('[DEBUG] Subscription required error detected');
        return NextResponse.json(
          { error: 'Subscription required', message: 'An active subscription is required to update tracker entries.' },
          { status: 403 }
        );
      }
